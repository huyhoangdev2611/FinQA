...
# -*- coding: utf-8 -*-
"""
Main script for testing the retriever model
"""
...
def generate(data_ori, data, model, ksave_dir, mode='valid'):
    ...
    if mode == "valid":
        ...
        # private data mode
        print_res = retrieve_evaluate_private(all_logits, all_filename_id, all_ind,
                                              output_prediction_file, conf.test_file, topn=conf.topn)
...
def generate_test():
    ...
    # Load chỉ phần trọng số model từ checkpoint
    checkpoint = torch.load(conf.saved_model_path, map_location=conf.device)
